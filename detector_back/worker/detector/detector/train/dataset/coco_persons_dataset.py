...
COCOPERSONS_CLASSES = ('__background__', 'person')

# for making bounding boxes pretty
COLORS = ((255, 0, 0, 128), (0, 255, 0, 128))


class COCOPersonsDetection(data.Dataset):
    def __init__(self, root, image_sets, preproc=None):
        ...
        if self.image_set == "train":
            ...
            self.table = val
        #self.table = self.table.iloc[random.sample(range(len(self.table)), 20000)]
    ...
    def pull_bbox(self, index):
        target = np.array(literal_eval(self.table.iloc[index].boxes))
        # add sintetic labels
